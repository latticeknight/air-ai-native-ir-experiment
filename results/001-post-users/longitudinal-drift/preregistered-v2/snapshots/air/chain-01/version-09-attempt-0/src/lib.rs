...
fn dispatch(request: Request) -> Value {
    ...
    if request.method == "PATCH" {
        return update_user(&request.path, &request.actor_role, &request.body);
    }
...
fn update_user(path: &str, actor_role: &str, body: &Value) -> Value {
    let Some(id) = path
        .strip_prefix("/users/")
    ...
    if body.contains_key("email") {
        ...
        if current.get("ok") != Some(&Value::Bool(true)) {
            return match current.get("error").and_then(Value::as_str) {
                Some("not_found") => response(404, json!({ "error": "not_found" })),
                _ => response(500, json!({ "error": "storage_failure" })),
            };
        }
        let Some(old_email) = current["user"].get("email").and_then(Value::as_str) else {
            return response(500, json!({ "error": "storage_failure" }));
        };
        let stored = call_operation(update_email, &json!({ "id": id, "email": email }));
        if stored.get("ok") == Some(&Value::Bool(true)) {
            let audited = call_operation(
                append_email_change,
                &json!({ "user_id": id, "old_email": old_email, "new_email": email }),
            );
            if audited.get("ok") != Some(&Value::Bool(true)) {
                return response(500, json!({ "error": "storage_failure" }));
            }
            return response(200, json!({ "id": id, "email": email }));
        }
        if let Some(error) = stored.get("error").and_then(Value::as_str) {
            return match error {
                "duplicate_email" => response(409, json!({ "error": "duplicate_email" })),
                "not_found" => response(404, json!({ "error": "not_found" })),
                _ => response(500, json!({ "error": "storage_failure" })),
            };
        }
        return response(500, json!({ "error": "storage_failure" }));
...
fn lookup_user(path: &str) -> Value {
    ...
    if stored.get("ok") == Some(&Value::Bool(true)) {
        response(200, public_user_fields(&stored["user"]))
    } else if stored.get("error").and_then(Value::as_str) == Some("not_found") {
        response(404, json!({ "error": "not_found" }))
    } else {
        response(500, json!({ "error": "storage_failure" }))
...
fn public_user_fields(user: &Value) -> Value {
    json!({
        "id": user["id"],
        "name": user["name"],
        "email": user["email"],
