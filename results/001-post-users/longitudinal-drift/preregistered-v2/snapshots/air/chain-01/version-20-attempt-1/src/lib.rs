...
// Endpoint imports are limited to the declared table-scoped capabilities; outbound network access is prohibited.
...
fn dispatch(request: Request) -> Value {
    ...
    if request.method == "PUT"
        && request
            .path
            .strip_prefix("/users/")
            .and_then(|path| path.strip_suffix("/profile"))
            .is_some()
    {
...
fn update_user_status(path: &str, actor_role: &str, body: &Value) -> Value {
    ...
    if actor_role != "administrator" && actor_role != "admin" {
    ...
    if body.len() > 2 || body.keys().any(|key| key != "status" && key != "reason") {
        return response(400, json!({ "error": "invalid_json" }));
    }
    let operation_input = if body.contains_key("reason") {
...
fn update_user(path: &str, _actor_role: &str, body: &Value) -> Value {
    let Some(id) = path
        .strip_prefix("/users/")
        .and_then(|value| value.parse::<u64>().ok())
    else {
...
fn lookup_user(path: &str) -> Value {
    ...
    if stored.get("ok") == Some(&Value::Bool(true)) {
        ...
        if is_deleted_user(user) {
            ...
            response(200, public_user_fields(user))
        }
    } else if stored.get("error").and_then(Value::as_str) == Some("not_found") {
        response(404, json!({ "error": "not_found" }))
    } else {
...
fn is_deleted_user(user: &Value) -> bool {
