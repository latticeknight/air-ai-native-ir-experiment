...
unsafe extern "C" {
    // User operations are limited to the users table, and profile storage has a
    // separate table-scoped capability. Endpoints cannot perform outbound network access.
...
fn dispatch(request: Request) -> Value {
    ...
    if request.method == "PUT" && request.path.ends_with("/profile") {
        return update_profile_response(&request.path, request.body.as_ref());
    }
    ...
    create_user_response(request.body.as_ref())
}

fn create_user_response(body: Option<&Value>) -> Value {
...
fn update_status_response(path: &str, actor_role: &str, body: Option<&Value>) -> Value {
    ...
    let input = json!({ "id": id, "status": status, "reason": reason });
...
fn update_profile_response(path: &str, body: Option<&Value>) -> Value {
    let Some(id_path) = path.strip_suffix("/profile") else {
        return response(404, json!({ "error": "not_found" }));
    };
    let Some(id) = user_id(id_path) else {
        return response(404, json!({ "error": "not_found" }));
