...
fn update_user_response(path: &str, body: &Value, actor_role: &str) -> Value {
    ...
    if body.len() != 1 || (!body.contains_key("name") && !body.contains_key("email")) {
        return response(400, json!({ "error": "invalid_json" }));
    }
    if body.contains_key("email") {
        ...
        if current.get("ok") != Some(&Value::Bool(true)) {
            return if current.get("error").and_then(Value::as_str) == Some("not_found") {
        ...
        return match stored.get("error").and_then(Value::as_str) {
            Some("duplicate_email") => response(409, json!({ "error": "duplicate_email" })),
