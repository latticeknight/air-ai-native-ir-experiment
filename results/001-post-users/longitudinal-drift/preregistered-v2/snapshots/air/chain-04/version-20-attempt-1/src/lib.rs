...
mod validation;
use validation::{validate_email, validate_name};
...
fn dispatch(request: Request) -> Value {
    ...
    if request.method == "PUT" && profile_id(&request.path).is_some() {
        return upsert_user_profile_response(&request.path, &request.body);
    }
    if request.method == "GET" {
        return get_user_response(&request.path);
    }
    if request.method == "PATCH" {
        if user_status_id(&request.path).is_some() {
            return update_user_status_response(&request.path, &request.body, &request.actor_role);
        }
        return update_user_response(&request.path, &request.body, &request.actor_role);
    }
    if request.method == "DELETE" {
        return soft_delete_user_response(&request.path, &request.actor_role);
    }
...
fn upsert_user_profile_response(path: &str, body: &Value) -> Value {
    let Some(id) = profile_id(path) else {
        return response(404, json!({ "error": "not_found" }));
    };
...
fn soft_delete_user_response(path: &str, actor_role: &str) -> Value {
    ...
    if stored.get("ok") == Some(&Value::Bool(true)) {
        response(200, json!({ "id": id, "deleted": true }))
...
fn update_user_status_response(path: &str, body: &Value, actor_role: &str) -> Value {
    let Some(id) = user_status_id(path) else {
        return response(404, json!({ "error": "not_found" }));
    };
    if !is_administrator(actor_role) {
        return response(403, json!({ "error": "forbidden" }));
    ...
    if body.len() > 2
        ...
        || body.keys().any(|key| key != "status" && key != "reason")
    {
        return response(400, json!({ "error": "invalid_json" }));
    }
    ...
    let reason = body
        .get("reason")
        .map(|value| value.as_str().filter(|reason| !reason.is_empty()));
    if reason == Some(None) {
        return response(400, json!({ "error": "invalid_json" }));
    }

    let mut operation_input = json!({ "id": id, "status": status });
    if let Some(reason) = reason.flatten() {
        operation_input["reason"] = json!(reason);
    }
    let stored = call_operation(update_status, &operation_input);
    if stored.get("ok") == Some(&Value::Bool(true)) {
...
fn get_user_response(path: &str) -> Value {
    ...
    if stored.get("ok") == Some(&Value::Bool(true)) {
        ...
        let Some(public_user) = public_user(user) else {
            return response(500, json!({ "error": "storage_failure" }));
        };
        response(200, public_user)
    } else if stored.get("error").and_then(Value::as_str) == Some("not_found") {
        response(404, json!({ "error": "not_found" }))
    } else {
        response(500, json!({ "error": "storage_failure" }))
...
fn user_id(path: &str) -> Option<u64> {
    ...
    if id.is_empty() || id == "0" || !id.bytes().all(|byte| byte.is_ascii_digit()) {
        return None;
    }
    id.parse::<u64>().ok()
}

fn user_status_id(path: &str) -> Option<u64> {
...
fn profile_id(path: &str) -> Option<u64> {
    let id = path.strip_prefix("/users/")?.strip_suffix("/profile")?;
    if id.is_empty() || id == "0" || !id.bytes().all(|byte| byte.is_ascii_digit()) {
        return None;
    }
    id.parse::<u64>().ok()
...
fn public_user(user: &serde_json::Map<String, Value>) -> Option<Value> {
    Some(json!({
        "id": user.get("id")?,
        "name": user.get("name")?,
