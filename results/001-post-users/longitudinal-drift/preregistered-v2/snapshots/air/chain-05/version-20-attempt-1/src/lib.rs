...
unsafe extern "C" {
    fn upsert_profile(input_ptr: i32, input_len: i32, output_ptr: i32, output_capacity: i32) -> i32;
}
...
fn dispatch(request: Request) -> Value {
    ...
    if request.method == "PUT" && request.path.ends_with("/profile") {
        return update_user_profile_response(&request.path, &request.body);
    }
...
fn update_user_profile_response(path: &str, body: &Value) -> Value {
    let Some(id) = path
        .strip_prefix("/users/")
    ...
    let stored = call_operation(upsert_profile, &json!({ "user_id": id, "timezone": timezone }));
...
fn update_user_status_response(path: &str, actor_role: &str, body: &Value) -> Value {
    ...
    if body.len() > 2 || (body.len() == 2 && !body.contains_key("reason")) {
        return response(400, json!({ "error": "invalid_json" }));
    }
    let update = match body.get("reason") {
        Some(reason) if reason.as_str().is_some_and(|reason| !reason.is_empty()) => {
            json!({ "id": id, "status": status, "reason": reason })
        }
        Some(_) => return response(400, json!({ "error": "invalid_json" })),
