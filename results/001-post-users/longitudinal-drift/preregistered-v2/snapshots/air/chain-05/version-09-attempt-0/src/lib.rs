...
fn update_user_response(path: &str, body: &Value, actor_role: &str) -> Value {
    ...
    if actor_role == "administrator" && body.len() == 1 && body.contains_key("email") {
        let Some(email) = body.get("email").and_then(Value::as_str) else {
            return response(400, json!({ "error": "invalid_json" }));
        };
        if !valid_email(email) {
        ...
        let Some(old_email) = current["user"]["email"].as_str() else {
            return response(500, json!({ "error": "storage_failure" }));
        };
        ...
        if updated.get("ok") == Some(&Value::Bool(true)) {
            ...
            // Keep the audit operation result and metadata out of the response.
