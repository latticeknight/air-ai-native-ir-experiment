...
Input {id,status,reason}. For status `suspended`, reason is required and must be a non-empty string.
The reason is stored internally and is never included in public responses.
