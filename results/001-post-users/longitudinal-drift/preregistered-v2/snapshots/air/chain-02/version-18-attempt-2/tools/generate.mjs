...
function validate(value) {
  ...
  if (value.network_policy?.allow_network_imports !== false || value.network_policy?.allow_outbound_network !== false) {
    throw new Error("network policy must prohibit imports and outbound network access");
  }
  const imports = new Set();
  for (const capability of value.capabilities) {
    const key = `${capability.module}.${capability.name}`;
    if (!capability.id || !capability.module || !capability.name || imports.has(key) || networkCapability(key)) {
...
function networkCapability(value) {
  return /(?:^|[._:-])(network|net|socket|tcp|udp|http|https|fetch|dns)(?:$|[._:-])/i.test(value);
}
...
function openApi(value, contractSha256) {
  ...
  const properties = Object.fromEntries(
    value.public_user_fields.map((field) => [field, fieldSchema(field)]),
  );
  return {
    openapi: "3.1.0",
