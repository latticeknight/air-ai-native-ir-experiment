...
fn upsert_user_profile(path: &str, body: &Value) -> Value {
    ...
    } else if stored.get("error").and_then(Value::as_str) == Some("user_suspended") {
        response(403, json!({ "error": "user_suspended" }))
    } else if stored.get("error").and_then(Value::as_str) == Some("not_found") {
        response(404, json!({ "error": "not_found" }))
    } else {
        response(500, json!({ "error": "storage_failure" }))
...
fn suspended_user(stored: &Value) -> bool {
    let Some(user) = stored.get("user").and_then(Value::as_object) else {
        return stored.get("status").and_then(Value::as_str) == Some("suspended")
            || stored.get("suspended").and_then(Value::as_bool) == Some(true)
            || stored.get("is_suspended").and_then(Value::as_bool) == Some(true);
    };

    user.get("status").and_then(Value::as_str) == Some("suspended")
        || user.get("suspended").and_then(Value::as_bool) == Some(true)
        || user.get("is_suspended").and_then(Value::as_bool) == Some(true)
}
