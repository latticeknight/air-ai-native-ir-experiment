...
fn update_status_user(path: &str, actor_role: &str, body: &Value) -> Value {
    ...
    if !administrator_role(actor_role) {
...
fn suspended_user(stored: &Value) -> bool {
    stored
        ...
        .or_else(|| stored.get("status").and_then(Value::as_str))
        == Some("suspended")
        || stored
            .get("user")
            .and_then(Value::as_object)
            .and_then(|user| user.get("suspended"))
            .and_then(Value::as_bool)
            == Some(true)
        || stored
            .get("user")
            .and_then(Value::as_object)
            .and_then(|user| user.get("is_suspended"))
            .and_then(Value::as_bool)
            == Some(true)
...
fn administrator_role(role: &str) -> bool {
    matches!(role, "administrator" | "admin")
}
