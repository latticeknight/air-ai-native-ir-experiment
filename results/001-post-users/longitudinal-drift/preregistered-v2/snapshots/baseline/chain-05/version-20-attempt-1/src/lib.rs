...
fn dispatch(request: Request) -> Value {
    if request.method == "PUT" && is_profile_path(&request.path) {
        return update_profile_response(&request.path, &request.body);
    }
    if request.method == "GET" {
        if normalized_path(&request.path) == "/health" {
...
fn is_profile_path(path: &str) -> bool {
    normalized_path(path).ends_with("/profile")
}

fn update_profile_response(path: &str, body: &Value) -> Value {
...
fn is_status_path(path: &str) -> bool {
    normalized_path(path).ends_with("/status")
}
...
fn update_status_response(path: &str, body: &Value, actor_role: &str) -> Value {
    ...
    input["reason"] = Value::String(reason.into());
