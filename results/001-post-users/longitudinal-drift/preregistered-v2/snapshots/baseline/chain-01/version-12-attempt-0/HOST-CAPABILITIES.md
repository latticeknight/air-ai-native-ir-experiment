...
Input {id}. Returns {ok:true,user} including internal storage fields, or {ok:false,error:not_found}. A user with status `deleted` is treated as not found by normal guest lookup. The guest must filter public output.
