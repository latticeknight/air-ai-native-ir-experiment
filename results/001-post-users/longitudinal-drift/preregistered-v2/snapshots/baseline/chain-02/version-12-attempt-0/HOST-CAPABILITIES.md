...
Input {id}. Returns {ok:true,user} including internal storage fields, or {ok:false,error:not_found}. The guest must filter public output and must treat a returned user with status `deleted` as not found.
