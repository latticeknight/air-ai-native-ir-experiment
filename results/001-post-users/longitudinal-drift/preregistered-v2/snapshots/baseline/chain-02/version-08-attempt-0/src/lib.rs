...
fn update_user(path: &str, body: &Value, actor_role: &str) -> Value {
    ...
    if body.len() == 1 && body.contains_key("email") {
        ...
        let old_email = if current.get("ok") == Some(&Value::Bool(true)) {
            let Some(user) = current.get("user").and_then(Value::as_object) else {
                return response(500, json!({ "error": "storage_failure" }));
            };
            let Some(old_email) = user.get("email").and_then(Value::as_str) else {
                return response(500, json!({ "error": "storage_failure" }));
            };
            String::from(old_email)
        ...
        let updated = call_operation(update_email, &json!({ "id": id, "email": email }));
        ...
        if updated.get("error").and_then(Value::as_str) == Some("duplicate_email") {
            return response(409, json!({ "error": "duplicate_email" }));
        }
        if updated.get("error").and_then(Value::as_str) == Some("not_found") {
            return response(404, json!({ "error": "not_found" }));
        }
        return response(500, json!({ "error": "storage_failure" }));
    }
