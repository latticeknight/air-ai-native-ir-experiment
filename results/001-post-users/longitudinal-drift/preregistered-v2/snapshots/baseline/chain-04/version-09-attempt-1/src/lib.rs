...
const PUBLIC_USER_FIELDS: [&str; 4] = ["id", "name", "email", "verified"];
...
fn update_user(path: &str, body: &Value, actor_role: &str) -> Value {
    ...
    if actor_role != "administrator" || !body.contains_key("email") {
        return response(400, json!({ "error": "invalid_json" }));
    }
    let Some(email) = body.get("email").and_then(Value::as_str) else {
        return response(400, json!({ "error": "invalid_json" }));
...
fn updated_user_response() -> Value {
    // The update and audit operation results are internal implementation data.
    json!({ "ok": true })
}
