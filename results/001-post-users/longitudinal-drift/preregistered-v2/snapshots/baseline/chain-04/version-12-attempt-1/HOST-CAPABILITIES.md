...
Input {id}. Returns {ok:true,user} including internal storage fields, or {ok:false,error:not_found}. A returned user with internal status `deleted` is hidden by normal guest lookup. The guest must filter public output.
