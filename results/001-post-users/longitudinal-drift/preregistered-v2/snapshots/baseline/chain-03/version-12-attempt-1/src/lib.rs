...
#[cfg(test)]
mod tests {
    use super::{is_deleted_user, public_user, valid_name};
    use serde_json::json;
    ...
    fn recognizes_soft_deleted_users() {
        ...
        assert!(is_deleted_user(deleted.as_object().expect("object")));
        assert!(is_deleted_user(status_deleted.as_object().expect("object")));
        assert!(!is_deleted_user(active.as_object().expect("object")));
    }
}
