    ...
    D: Deserializer<'de>,
{
    ...
    match value {
        ...
        Value::Object(actor) => actor_role_from_object(&actor)
            .map(String::from)
...
fn actor_role_from_object(actor: &serde_json::Map<String, Value>) -> Option<&str> {
    actor
        .get("role")
        ...
        .or_else(|| {
            actor
                .get("claims")
                .or_else(|| actor.get("principal"))
                .or_else(|| actor.get("identity"))
                .and_then(Value::as_object)
                .and_then(actor_role_from_object)
        })
}
