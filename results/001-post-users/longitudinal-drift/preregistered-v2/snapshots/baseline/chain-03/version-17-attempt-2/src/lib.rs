...
// Endpoints are limited to the declared air_users_v1 storage imports and never
...
mod tests {
    ...
    fn counts_unicode_scalars_instead_of_utf8_bytes() {
        assert!(validation::name(&"é".repeat(100)));
        assert!(!validation::name(&"é".repeat(101)));
    }
